...
void go_back_n(struct sockaddr_in* client_add, int socket_no, char* message_buff){
    struct Window w;
    InitWindow(&w);
    ...
    while(1){
        ...
        if(sended_index < send_size){
            if(w.slot){
                ...
                sendto(socket_no, (const struct UserDatagramProtocol*) pack, pack_size, MSG_CONFIRM, (const struct sockaddr *)client_add, sizeof(*client_add));
        ...
        if(fd[0].revents & POLLIN){
            ...
            recvfrom(socket_no, (struct UserDatagramProtocol *)new_receive, MAXLEN, MSG_WAITALL, (struct sockaddr *)client_add, sizeof(*client_add));
            if (strcmp(pack->app_data, "\n") == 0 )
            {
                empty_new_lines++;
            }
            ...
            if(!is_check_sum_ok){   //If checksum is not same wait till timout to resend of data
                if(!new_receive->ACK){
                    if((new_receive->sequence_no)%16==w.next_seq_no){
                        ...
                        sendto(socket_no, (const struct UserDatagramProtocol*)new_receive, sizeof(*new_receive), MSG_CONFIRM, (const struct sockaddr *)client_add, sizeof(*client_add));
                        old_received_pack = new_receive;

                    }
                    else{
                        sendto(socket_no, (const struct UserDatagramProtocol*)old_received_pack, sizeof(*old_received_pack), MSG_CONFIRM, (const struct sockaddr *)client_add, sizeof(*client_add));
                    }
                }
                else{
        ...
        if(w.slot!=16){
            ...
            if(time_interval>TIMEOUT){
                ...
                for(int i=0;i<(16-w.slot);i++){
                    ...
                    sendto(socket_no, (const struct UserDatagramProtocol*)resend_pack, sizeof(*resend_pack), MSG_CONFIRM, (const struct sockaddr *)client_add, sizeof(*client_add));
                    temp = (temp+1)%16;
                }
            }
        }
...
int main(int argc, char * argv[]){
    ...
    int is_bind= bind(socketfd, &server_add, sizeof(server_add));
    if (is_bind<0){
    ...
    go_back_n(&client_add, socketfd, message_buff);
